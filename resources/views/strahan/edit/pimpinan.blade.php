...
@section('content')
    ...
    <main class="flex-grow flex flex-col items-center py-10 px-4">
        ...
        <div class="bg-white/90 rounded-lg shadow-lg p-8 w-full max-w-md border-t-4 border-blue-700">
            {{-- LOGIN SECTION --}}
            <h3 class="text-xl font-semibold text-center text-blue-700 mb-4">Input Pimpinan</h3>

            <form action="/strahan/input-info" method="POST" class="space-y-5">
                @csrf
                <div>
                    <div class="grid grid-cols-1 md:grid-cols-2 items-center gap-3">
                    <label class="font-bold text-black">Upload Foto</label>
                    <input name="foto" type="file" class="border border-green-400 rounded-md px-2 py-1 w-full">
                </div>
